...
while True:
    ...
    frame2 = np.zeros((480, 320, 3), np.uint8)
    ...
    remask2 = cv2.merge((remask, remask, remask))
    h2, w2, _ = frame.shape
    frame2[0:h2, 0:w2] = frame
    h2, w2, _ = frame.shape
    frame2[0:h2, 160:160+w2] = remask2
    ...
    for pic, contour in enumerate(contours_color):
        c_color = max(contours_color, key=cv2.contourArea)
